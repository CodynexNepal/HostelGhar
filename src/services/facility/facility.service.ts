...
import {
  ...
  FacilityUpsertInput,
  toFacilitySlug,
} from '../../repository/facility/facility.repository';
...
export const isUuidV4 = (v: unknown): v is string =>
  typeof v === 'string' && UUID_V4.test(v.trim());
const normTag = (t: unknown): FacilityTag => {
  const s = typeof t === 'string' ? t.trim().toLowerCase() : '';
  if (s === 'excluded') return FacilityTag.EXCLUDED;
  if (s === 'extra charge' || s === 'extra_charge' || s === 'extra-charge')
    return FacilityTag.EXTRA_CHARGE;
...
export class FacilityService {
  ...
  private shape(
    rows: Awaited<ReturnType<FacilityRepository['listByHostel']>>,
  ): FacilityResponseItem[] {
    return rows.map((r) => ({
      id: r.id,
      title: r.facility?.title ?? '',
  ...
  private async hostelExists(hostelId: string): Promise<boolean> {
    const h = await AppDataSource.getRepository(Hostel).findOne({
      where: { id: hostelId },
      select: { id: true },
    });
    return !!h;
  ...
  public async list(hostelId: string) {
    if (!isUuidV4(hostelId))
      return {
        error: { status: STATUS_CODE.BAD_REQUEST, message: 'Invalid hostel id — must be UUID v4' },
      } as const;
    const cacheKey = this.key(hostelId);
    const wrapped = await cacheService.wrap(
      cacheKey,
      async () => {
        if (!(await this.hostelExists(hostelId)))
          return { missing: true as const, rows: [] as FacilityResponseItem[] };
        return {
          missing: false as const,
          rows: this.shape(await this.repo.listByHostel(hostelId)),
        };
      },
      { l1TtlSeconds: 30, l2TtlSeconds: 120 },
    );
    if (wrapped.data.missing)
      return { error: { status: STATUS_CODE.NOT_FOUND, message: 'Hostel not found' } } as const;
    return {
      data: wrapped.data.rows,
  ...
  public async sync(hostelId: string, items: FacilityUpsertInput[]) {
    ...
    const bySlug = new Map<
      string,
      { title: string; description: string | null; tag: FacilityTag; clientKey: string | null }
    >();
    ...
    const miss = slugs
      ...
      .map((s) => ({ title: bySlug.get(s)!.title, slug: s }));
    if (miss.length > 0)
      for (const c of await this.repo.createCatalogEntries(miss)) catBySlug.set(c.slug, c);
    ...
    for (const [slug, v] of bySlug) {
      ...
      if (row) {
        let dirty = false;
        if ((row.description ?? null) !== v.description) {
        ...
        if (v.clientKey && row.clientKey !== v.clientKey) {
    ...
    await eventDispatcher
      .dispatch({
        ...
        payload: { hostelId, facilities: data },
        hostelId,
        metadata: { hostelId, f: data.length },
      })
      .catch(() => undefined);
  ...
  public async add(hostelId: string, input: FacilityUpsertInput) {
    ...
    const merged: FacilityUpsertInput[] = cur.data.map((f) => ({
    ...
    return {
      data: res.data.find((d) => d.slug === slug) ?? res.data[res.data.length - 1],
    } as const;
  }
  public async remove(hostelId: string, fkey: string) {
    ...
    if (!k)
      return {
        error: { status: STATUS_CODE.BAD_REQUEST, message: 'Facility id is required' },
      } as const;
    if (!(await this.hostelExists(hostelId)))
      return { error: { status: STATUS_CODE.NOT_FOUND, message: 'Hostel not found' } } as const;
    const target = (await this.repo.findJunctionByHostel(hostelId)).find(
      (r) => r.id === k || (r.clientKey ?? '') === k,
    );
    if (!target)
      return {
        error: { status: STATUS_CODE.NOT_FOUND, message: 'Facility not found for this hostel' },
      } as const;
